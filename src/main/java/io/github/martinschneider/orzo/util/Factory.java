...
public class Factory {
	public static Expression expr(String input) throws IOException {
		return new ExpressionParser(ParserContext.build(new CompilerErrors())).parse(new Lexer().getTokens(input));
	}
...
//
	...
	public static ArrayInit arrInit(String type, List<Expression> dimensions, List<List<Expression>> vals) {
	...
	public static ArrayInit arrInit(String type, int dim, List<Expression> vals) throws IOException {
		return arrInit(type, list(expr(list(Token.of(INT_LITERAL, String.valueOf(dim))))), list(vals));
	}
...
//
  ...
  public static Identifier id(String val, ArraySelector arrSel) {
  ...
  public static Identifier id(String... vals) {
	    Identifier id = Identifier.of(vals[0]);
	    Identifier root = id;
	    for (int i = 1; i < vals.length; i++) {
	      id.next = Identifier.of(vals[i]);
	      id = id.next;
	    }
	    return root;
	  }
