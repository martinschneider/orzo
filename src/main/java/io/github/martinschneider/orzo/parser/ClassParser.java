...
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.CLASS;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.ENUM;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.EXTENDS;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.IMPLEMENTS;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.IMPORT;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.INTERFACE;
import static io.github.martinschneider.orzo.lexer.tokens.Keyword.PACKAGE;
...
public class ClassParser implements ProdParser<Clazz> {
	...
	public Clazz parse(TokenList tokens) {
		...
		if (tokens.curr().eq(KEYWORD, INTERFACE)) {
			...
			isEnum = true;
		} else if (tokens.curr().eq(KEYWORD, CLASS) || isInterface || isEnum) {
			tokens.next();
			if (tokens.curr().isId()) {
				name = tokens.curr().val.toString();
			} else {
				name = null;
				ctx.errors.addError(LOG_NAME, "missing identifier", new RuntimeException().getStackTrace());
			}
			tokens.next();
			...
			if (!tokens.curr().eq(sym(LBRACE))) {
				...
				ctx.errors.missingExpected(LOG_NAME, sym(LBRACE), tokens, new RuntimeException().getStackTrace());
				return null;
			}
			tokens.next();
			// set temporary clazz object for the method parser's use
			ctx.currClazz = new Clazz(packageDeclaration, imports, scope, name, isInterface, isEnum, interfaces,
					baseClass, null, null, null);
			if (isEnum) {
			...
			if (members == null) {
				ctx.errors.addError(LOG_NAME, "missing class body", new RuntimeException().getStackTrace());
			}
			...
			// TODO: one could argue that this should be handled during code generation
			// rather than
			// parsing
			...
			if (!tokens.curr().eq(sym(RBRACE))) {
				ctx.errors.tokenIdx = tokens.idx();
				ctx.errors.missingExpected(LOG_NAME, sym(RBRACE), tokens, new RuntimeException().getStackTrace());
			}
	...
	private String parseBaseClass(TokenList tokens, boolean isEnum) {
		if (tokens.curr().eq(keyword(EXTENDS))) {
			tokens.next();
			if (tokens.curr().isId()) {
				String baseClass = tokens.curr().val;
				tokens.next();
				return baseClass;
			}
		}
	...
	List<String> parseInterfaces(TokenList tokens, List<String> interfaces) {
		if (tokens.curr().eq(keyword(IMPLEMENTS))) {
			tokens.next();
			while (!tokens.curr().eq(keyword(EXTENDS)) && !(tokens.curr().eq(sym(LBRACE)))
					&& !(tokens.curr().isEOF())) {
				if (tokens.curr().isId()) {
					interfaces.add(tokens.curr().val);
				}
				tokens.next();
	...
	Import parseImport(TokenList tokens) {
		...
		if (tokens.curr().eq(keyword(IMPORT))) {
			...
			while (!tokens.curr().eq(sym(SEMICOLON)) && !(tokens.curr().isEOF())) {
		...
		if (tokens.curr().eq(keyword(PACKAGE))) {
			...
			while (!tokens.curr().eq(sym(SEMICOLON)) && !(tokens.curr().isEOF())) {
				if (tokens.curr().eq(sym(DOT))) {
					...
					ctx.errors.addError(LOG_NAME, "invalid token " + tokens.curr() + " in package declaration",
							new RuntimeException().getStackTrace());
